...
%%% 13. 1.1997, Gernot Salzer: initial version
%%% ??.11.1997, Wolfram Nix: preliminaries, lots of definitions, theorems etc added.
%%% 31.10.2020, Richard Zach: Bring this into the 21st century! Also, add tableaux
%%%  9. 5.2021, Gernot Salzer: appendix with specification of logic
...
\usepackage{doi}
\renewcommand*{\doi}[1]{\href{https://dx.doi.org/#1}{DOI #1}}
...
\author{M.~Ultlog\thanks{Generated by \href{https://github.com/rzach/multlog}{\textsc{multlog}},
v.~@ml_version@ from the specification of \NameOfLogic\ logic in the appendix.}}
\date{}

\makeatletter
\hypersetup{pdftitle={\@title},pdfauthor={\@author},pdfkeywords={{\NameOfLogic}
logic, sequent calculus, tableaux, natural deduction, resolution}}
\makeatother
...
clause translation systems for resolution for \NameOfLogic\ logic.
\end{abstract}
...
\emakefirstuc{\NameOfLogic} logic has \numtoname{\NoTVs} truth values
...
tableau system for \NameOfLogic\ logic.
...
the natural deduction system for \NameOfLogic\ logic in
\cref{sec:natural}.
...
\ifprop
   The \emph{propositional language~\LL\/} for \NameOfLogic{} logic consists of
   \begin{enumerate}
   ...
   \ifnum\NoOps=0 
      ...
      called \emph{atomic} or an \emph{atom}. \NameOfLogic\ logic only
   ...
   \fi 
\else
   The \emph{first order language~\LL\/} for \NameOfLogic{} logic
   consists of
   \begin{enumerate}
...
%%% Matrix
\begin{definition}
The \emph{matrix} for \NameOfLogic\ logic is given by:
\begin{enumerate}
...
\section{Sequent calculus for \NameOfLogic\ Logic}\label{sec:sequent}
...
\begin{definition}
The \emph{sequent calculus} for \NameOfLogic\ logic is given by:
\begin{enumerate}
...
\begin{corollary}
   In \NameOfLogic\ logic, $\models A$ iff $[\DTVss\colon A]$ has a
...
\section{Tableaux for \NameOfLogic\ logic}\label{sec:tableaux}

Although the method of \citet{Surma1977} and \citet{Carnielli1987} for
obtaining signed analytic tableaux systems applies to \NameOfLogic{}
logic, it has a drawback.  As \citet{Hahnle1993} pointed out, to show
that a formula is valid, it is required to provide as many closed
tableaux as there are non-designated values.  This is usually not
desirable; the generalized approach by \citet{Hahnle1993} solves this
problem. Below we give a tableau system for \NameOfLogic{} logic using
the sets of signs $\TVs \setminus \{v\}$, i.e., the tableau system
exactly dual to that of Carnielli (in the sense of
\cite{BaazFermullerZach1993}).
...
\begin{corollary}
   In \NameOfLogic{} logic, $\models A$ iff $\{v \colon A \mid v \in
...
\section{Natural deduction for \NameOfLogic\ logic}\label{sec:natural}
...
\begin{definition}
The \emph{natural deduction rules} for \NameOfLogic\ logic are given by:
\begin{enumerate}
...
\section{Resolution and clause formation rules for \NameOfLogic\ logic}\label{sec:clauses}

The many-valued resolution calculus of \citet{BaazFermuller1995}
applies to \NameOfLogic\ logic. We present the framework here, as well
as a language preserving clause translation system for \NameOfLogic\ logic.
