...
\newcommand\ESeqOpprems[2]{\csname ESeqOpprems\romannumeral#1X\romannumeral#2\endcsname}
\newcommand\ESeqOpconcl [2]{\csname ESeqOpconcl\romannumeral#1X\romannumeral#2\endcsname}
...
\newcommand\TabQuprem [2]{\csname
TabQuprem\romannumeral#1X\romannumeral#2\endcsname}
\newcommand\ESeqQuprems[2]{\csname ESeqQuprems\romannumeral#1X\romannumeral#2\endcsname}
\newcommand\ESeqQuconcl [2]{\csname ESeqQuconcl\romannumeral#1X\romannumeral#2\endcsname}
...
\rulesep=2em
\newbox\rulebox
\def\maxrulewidth{1.5\textwidth}
\newenvironment{rules}{\begin{center}\rulewidth=0pt}{\end{center}}
\def\setrule#1{%
   \setbox\rulebox\hbox{\adjustbox{max width=\maxrulewidth}{#1}}%
   \ifdim\wd\rulebox>\textwidth
      \ifdim\rulewidth>0pt\\[2ex]\fi
      \makebox[0pt]{\usebox\rulebox}%
      \rulewidth=\textwidth
   \else
      \ifdim\rulewidth>0pt
         \advance\rulewidth\rulesep
         \advance\rulewidth\wd\rulebox
         \ifdim\rulewidth>\textwidth
            \\[2ex]
            \rulewidth=\wd\rulebox
         \else
            \hskip\rulesep
         \fi
      \else
...
\def\TVss{\TV1\FOR\n=2\TO\NoTVs\DO,\allowbreak \TV\n \ENDFOR}
\newcommand\DTV[1]{\IfIsDTV{#1}\loopcomma\TV{#1}\def\loopcomma{,\allowbreak} \fi}%
\def\DTVss{\def\loopcomma{}%
   \FOR\n=1\TO\NoTVs\DO%
      \DTV{\n}
   \ENDFOR}
\newcommand\NDTV[1]{\IfIsDTV{#1}\else
   \loopcomma{}%
   \TV{#1}%
   \def\loopcomma{,\allowbreak}%
   \fi}%
...
\def\esequent#1{%
   \def\loopcomma{}%
   \def\do##1{\loopcomma ##1\def\loopcomma{\mid}}%
   \docsvlist{#1}}
...
[PDF generated by MULTLOG, v.~@ml_version@,
\url{https://logic.at/multlog}]. \texExtra{Link}{Available at
\url{\Link}.}{}\newline\newline See
...
\texExtra{ShortName}{\ensuremath{\ShortName}}{\emakefirstuc{\NameOfLogic}}
has \numtoname{\NoTVs} truth values $\TVss$ (with $\DTVss$
designated), \ifnum\NoQus=0 and
\fi
\ifnum\NoOps>0
   connective\PS{\NoOps}
   $\Opname1$%
...
automatically has soundness and completness theorems, cut-elimination
theorem\ifnotprop, mid-sequent theorem,\fi\ and Maehara lemma
(interpolation). For proofs of these results see
\cite{BaazFermullerZach1994,Zach1993}.
...
the natural deduction system for
\texExtra{ShortName}{\ensuremath{\ShortName}}{\NameOfLogic} in
\cref{sec:natural}.

In addition to H\"ahnle's work on tableaux-based theorem proving for
finite-valued logic, \citet{BaazFermuller1995} have studied resolution
calculi for clauses of signed literals. In order for these calculi to
be used to prove that formulas of \NameOfLogic\ are \ifprop valid\else
tautologies\fi{} or follow from some others, it is necessary to
produce sets of signed clauses. In \cref{sec:clauses}, we present a
translation calculus that yields a set of clauses from a set of
formulas. 
...
\ifprop
   \begin{definition}
   Let $A$ be a formula and $x_0, x_1, \ldots, x_k$ the variables
   occurring in $A$. Then an \emph{interpretation\/}~$\I$ of~$A$ is an
...
\else
   ...
   \item If $A=(\quantor x)(B(x))$, and $\tquantor$ is the
         ...
         $\val_{\I}(A)=\tquantor\bigl\{\val_{\I}(B(d))|d\in D\}\bigr)$.
   \end{enumerate}
   \end{definition}
...
If $\Gamma$ and $\Gamma'$ are sequents, then $\Gamma, \Gamma'$ denotes
the component-wise union, i.e., the $v$-component of $\Gamma, \Gamma'$
is $\Gamma_v, \Gamma'_v$.
\end{definition}
...
$\Gamma$ is called \emph{satisfiable\/} iff there is an
interpretation~$\I$ so that $\I \models \Gamma$ and \emph{valid\/} iff
for every interpretation~$\I$, $\I \models \Gamma$.
\end{definition}

\begin{proposition}
   $\Delta \models A$ iff the sequent 
   \ifESequents
      $\consequent{\Delta}{A}$
   \else
      $[\NDTVss\colon \Delta], [\DTVss\colon A]$
   \fi is valid.
\end{proposition}
...
\begin{enumerate}
\item axiom schemas of the form 
   \ifESequents
      $\consequent{A}{A}$%
   \else
      $[\TVs \colon A]$%
...
\FOR \op=1 \TO \NoOps \DO
   {\noindent
    ...
    \begin{rules}
       \FOR \tv=1 \TO \NoTVs \DO
         \ifESequents
            \setrule{\infer[\Opname\op{:}\TV\tv]{\ESeqOpconcl{\op}{\tv}}{\ESeqOpprems{\op}{\tv}}}%
         \else
            \setrule{\infer[\Opname\op{:}\TV\tv]{\Opconcl{\op}{\tv}}{\Opprems{\op}{\tv}}}%
         \fi
       \ENDFOR
    \end{rules}%
   }
...
\FOR \qu=1 \TO \NoQus \DO
   {\noindent
    ...
    \begin{rules}
       \FOR \tv=1 \TO \NoTVs \DO
          \ifESequents
            \setrule{\infer[\Quname\qu{:}\TV\tv]{\ESeqQuconcl{\qu}{\tv}}{\ESeqQuprems{\qu}{\tv}}}%
         \else
            \setrule{\infer[\Quname\qu{:}\TV\tv]{\Quconcl{\qu}{\tv}}{\Quprems{\qu}{\tv}}}%
         \fi
       \ENDFOR
    \end{rules}%
   }
...
\begin{corollary}
   In \NameOfLogic, $\models A$ iff 
   \ifESequents
      $\consequent{}{A}$
   \else
      $[\DTVss\colon A]$
   \fi has a
   sequent proof, and $\Delta \models A$ iff 
   \ifESequents
      $\consequent{\Delta}{A}$
   \else
      $[\NDTVss\colon \Delta], [\DTVss\colon A]$
...
\ifnotprop
\begin{theorem}[Midsequent theorem]
   The midsequent theorem holds in the sequent calculus for
   \NameOfLogic.
\end{theorem}

\begin{proof}
See Theorem 6.1 of \citet{BaazFermullerZach1994} or Theorem 3.7.2 of
\citet{Zach1993}.
\end{proof}
\fi

\begin{theorem}[Maehara lemma]
   The Maehara lemma holds for the sequent calculus for
   \NameOfLogic.
\end{theorem}

\begin{proof}
See Theorem 3.8.1 of \citet{Zach1993}.
\end{proof}
...
\begin{definition}
   ...
   above it by a branch expansion rule. A tableau is \emph{closed} if
...
\FOR \qu=1 \TO \NoQus \DO
   {\noindent
    The branch expansion rules rules for quantifier $\Quname\qu$ are given by
    \begin{rules}
       \FOR \tv=1 \TO \NoTVs \DO
          \setrule{$\begin{array}[t]{@{}c@{}}
...
\item A weakening rule for all $v \in \TVs^+$:
      ...
      \]
\ifnum\NoOps>0
   \item For every connective $\operator$ and every truth value $v$ an
         introduction rule $\operator\textsc{i}{:}v$ (if $v \in \TVs^+$) or an elimination
         rule $\operator\textsc{e}{:}v$ (if $v \in \TVs^-$).
\fi
\ifnum\NoQus>0
...
\FOR \op=1 \TO \NoOps \DO
   {\noindent
    The introduction and elimination rules for connective $\Opname\op$ are given by
       \begin{rules}
       \FOR \tv=1 \TO \NoTVs \DO
            \setrule{\infer[{\Opname\op}{\NDEI\tv} \colon {\TV\tv}]
            {\NDOpconcl{\op}{\tv}}
            {\NDOpprems{\op}{\tv}}}%
       \ENDFOR
    \end{rules}%
...
\ifprop Let $\I$ be an interpretation. $\I$~\emph{satisfies\/} a
clause~$C$ iff there is some signed formula $A^v \in C$, so that
$\val_\I(A) = v$. $\I$~satisfies a clause set~$\C$ iff it satisfies
every clause in~$\C$.  $\C$~is called \emph{satisfiable} iff some
structure satisfies it, and \emph{unsatisfiable} otherwise.\else Let
$\M$ be a structure. $\M$ \emph{satisfies\/} a clause~$C$ iff for
every assignment $s$, there is some signed formula $A^v \in C$, so
that $\val_\I(A) = v$ (where $\I = \langle \M,s\rangle$).
$\M$~satisfies a clause set~$\C$ iff it satisfies every clause
in~$\C$.  $\C$~is called \emph{satisfiable} iff some structure
satisfies it, and \emph{unsatisfiable} otherwise.\fi
\end{definition}
...
\begin{definition}
   A \emph{resolution refutation} of a clause set $\C$ is a
   sequence of clauses $C_1$, \dots, $C_n$ so that for every $i$, $C_i
   \in \C$ or $C_i$ is a resolvent of $C_j$, $C_k$ with $j, k <
   i$, and $C_n = \emptyset$. 
\end{definition}
